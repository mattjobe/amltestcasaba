#!/usr/bin/env python3
"""
Extended Exploitation and Write-Access Evaluation for Azure ML Compute
----------------------------------------------------------------------
...
Azure ML compute instance. It performs the following steps:
  ...
  3. If escape is successful, collects the same recon data from the host.
  4. Attempts to write temporary files to key host directories to test for
     modifiability.
  5. Compares the recon data and write-access results to compute a risk score.
  6. Outputs a comprehensive JSON report with recommendations.
...
def enumerate_sensitive_dirs():
    ...
    # Package listings (dpkg on Debian/Ubuntu)
...
def test_write_access(directory):
    ...
    """
    test_file = os.path.join(directory, "temp_write_test.txt")
    try:
...
def check_host_writeability():
    ...
    """
    directories = ["/etc", "/root", "/mnt/azureml", "/tmp"]
    writable = {}
...
def compute_risk_score(container_data, host_data, escape_success):
    ...
    if any(write_access.get(d) is True for d in write_access if isinstance(write_access.get(d), bool)):
        score += 3

    sensitive_dirs = host_data.get("sensitive_dirs", {})
    if any("Not found" not in str(v) for v in sensitive_dirs.values()):
        score += 2
...
def main():
    ...
    # 3. Compare container and host data.
    ...
    report["risk_assessment"] = {
        ...
        "recommendations": (
            ...
            "Writable host directories are a serious concern, as they may allow an attacker to modify critical system files."
            if level == "High" else
